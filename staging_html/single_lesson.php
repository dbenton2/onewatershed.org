...
<title>OneWatershed</title>
    ...
    <script type="text/javascript">
        function changeView(source, ID) {  
            document.getElementById("iframe").setAttribute("src", "http://docs.google.com/gview?url=http://onewatershed.org/staging_html/PDFs/" + source + "&embedded=true");
            loadMaterials(ID);
    ...
    function nextMonth(ID){
        ...
        if(nextMonth == 0){
           var prevYear = year-1;
           var nextDate = new Date(nextMonth +' 1 ,'+prevYear);
        }
        else{var nextDate = new Date(nextMonth +' 1 ,'+year);}
    ...
    function prevMonth(ID){
        ...
        // days in previous month and next one , and day of week.
        if(nextMonth == 11){
           var nextDate = new Date(nextMonth +' 1 ,'+year+'1');
        }
        else{var nextDate = new Date(nextMonth +' 1 ,'+year);}
...
if ($_SERVER['REQUEST_METHOD'] == 'POST'){
    $lessonName = test_input($_POST['lesson']);
}
else{
    $lessonName = test_input($_GET['lesson']);
}
?>
</ul>
<div>
...
<form method="post" action="reserve_date.php"><br><br>
<fieldset>
...
<input type="hidden" id="lessonName" value="<?php echo $lessonName; ?>" name="lessonName"/>
</fieldset><br>
...
echo "Sorry, there are no lessons currently available for this topic.";
   ...
   echo "<div><section class=''><div class='container'><table class='topic'>";
   ...
   echo "<tr id='" .$holder["PDF_link"]. "' onclick='changeView(\"" .$holder["PDF_link"]. "\", " .$holder["ID"]. ")'><td>" .$holder["Name"]. "</td>";
 ...
 echo "<div class='scheduling_materials'>";
	...
	echo "<div class='calendar' id='cal'>";
		...
		echo "<div class='cal_header'>";
    			...
    			echo "<span class='right button' onclick='nextMonth()'> &rang; </span>";
		echo "</div>";
