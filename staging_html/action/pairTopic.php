...
for ($x=0; $x < count($_POST); $x++){
    if ($_POST[$x] == 'on'){
        $uploadOk = 1;
        $topic = test_input($_POST['topicName'.$x]);
        $lessonID = $_POST['lessonID'.$x];
        ...
        if ($topicID == ''){
            $uploadOk = 0;
            echo "The lesson " .$lessonName." was not paired with the topic " .$topic. ". There was an issue finding the topic.<br>";
        }
        
        $sql2 = "INSERT INTO topic_lesson_junction (Topic_ID, Name_ID)
                 VALUES ('$topicID', '$lessonID')";
        
        if ($uploadOk == 1){         
            if ($con->query($sql2) === TRUE){
                echo "The lesson" .$lessonName." was successfully paired with the topic " .$topic. ".<br>";
            }
            else{
                echo "The lesson " .$lessonName." was not paired with the topic " .$topic. ". uploadOk was 1.<br>";
            }
        }
    }
}
