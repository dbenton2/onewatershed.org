...
<title>OneWatershed - Admin</title>
    ...
    <script>
    var topicResults = document.getElementsByClassName("topicResults");
    
    function topicSearch(name,x){
        if (name.length==0){
            document.getElementById("topicResults"+x).innerHTML = "";
            return;
        }
        xmlhttp = new XMLHttpRequest();
        xmlhttp.onreadystatechange=function(){
            if (this.readyState==4 && this.status==200){
                document.getElementById("topicResults"+x).innerHTML=this.responseText;
                document.getElementById("topicResults"+x).style.display = "block";
            }
        };
        xmlhttp.open("GET", "search/topicSearch.php?x="+name +"&q="+x, true);
        xmlhttp.send();
    }

    function fillTopic(name, x){
        var string = name;
        var final = string.toString();
        document.getElementById("searchTopic"+x).value = final;
        document.getElementById("topicResults"+x).style.display = "none";
    }
    $(document).on('click', function(event) {
        if (!$(event.target).closest('#searchTopic').length) {
         document.getElementsByClassName("topicResults").style.display = "none";
        }
    });
    </script>
    <script>    
...
if(isset($_SESSION['loggedin']) && $_SESSION['loggedin'] == true && $_SESSION['accessLevel'] == 'admin'){
    ...
    if ($result->num_rows > 0){
        ...
        echo "<form action='action.php' method='post'><section class=''><div class='lesson_container'><table class='lessons'>"; 
        ...
        while($row = $result->fetch_assoc()){
   	   echo "<tr class='results'><td><input type='checkbox' name='".$x. "'/></td>";
   	   echo "<input type='hidden' name='Theme".$x."' value='".$row["Recommended_Theme"]."'/>";
   	   echo "<input type='hidden' name='Topic".$x."' value='".$row["Recommended_Topic"]."'/>";
...
$host = 'localhost';
$user = 'upload_data';
$password = 'crazy_fish15!';
$dbname = 'onewatershed_live_data';

//connect to the database
$con = new mysqli($host, $user, $password, $dbname);

//if connection failed output to the user   
if ($con->connect_error) {
    die("Sorry, connection to the database failed. Please try again.<br> ");
}
